# Привет! Меня зовут Владислав 👋

Санкт-Петербургский iOS-разработчик, который верит, что хороший код должен быть не только функциональным, но и элегантным. Как хороший кофе — бодрит и вдохновляет ☕️

---

### 🤝 Как со мной связаться

[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/vladissann)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sannikovladislav@mail.ru)

---

### ⚡ Мой стек и инструменты

**iOS & Swift**
![Swift](https://img.shields.io/badge/swift-F54A2A?style=for-the-badge&logo=swift&logoColor=white)
![Xcode](https://img.shields.io/badge/Xcode-007ACC?style=for-the-badge&logo=Xcode&logoColor=white)
![UIKit](https://img.shields.io/badge/UIKit-2396F3?style=for-the-badge&logo=uikit&logoColor=white)
![SwiftUI](https://img.shields.io/badge/SwiftUI-0077B5?style=for-the-badge&logo=swift&logoColor=white)

**Архитектура и данные**
![MVC](https://img.shields.io/badge/MVC-007396?style=for-the-badge&logo=apple&logoColor=white)
![MVP](https://img.shields.io/badge/MVP-007396?style=for-the-badge&logo=apple&logoColor=white)
![MVVM](https://img.shields.io/badge/MVVM-007396?style=for-the-badge&logo=apple&logoColor=white)
![Core Data](https://img.shields.io/badge/Core%20Data-092E20?style=for-the-badge&logo=apple&logoColor=white)
![Realm](https://img.shields.io/badge/Realm-39477F?style=for-the-badge&logo=realm&logoColor=white)
![UserDefaults](https://img.shields.io/badge/UserDefaults-FF9900?style=for-the-badge&logo=apple&logoColor=white)

**Сеть и многопоточность**
![URLSession](https://img.shields.io/badge/URLSession-FF6C37?style=for-the-badge&logo=apple&logoColor=white)
![GCD](https://img.shields.io/badge/GCD-007396?style=for-the-badge&logo=apple&logoColor=white)

**Инструменты**
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)

---

### 👯 Я открыт к сотрудничеству

*   В интересных **open-source** iOS-проектах, где можно учиться у сообщества.
*   В разработке **пет-проектов** для портфолио и души.
*   К **менторству** и обмену опытом с такими же начинающими разработчиками.

---

### 📌 Мои проекты

*   [**TravelSchedule**](https://github.com/SannikovVladislav/TravelSchedule) — приложение для поиска и планирования путешествий (активная разработка 🚧).
*   [**MovieQuiz**](https://github.com/SannikovVladislav/MovieQuiz) — увлекательный квиз для киноманов на Swift и UIKit.
*   [**Tracker**](https://github.com/SannikovVladislav/Tracker) — трекер привычек с кастомным UI и хранением на Core Data.
*   [**ImageFeed**](https://github.com/SannikovVladislav/ImageFeed) — бесконечная лента изображений, работа с сетью и многопоточностью.
*   [**CreatureCards**](https://github.com/SannikovVladislav/CreatureCards) — детская карточная игра для изучения животных.
*   [**Counter**](https://github.com/SannikovVladislav/Counter) — простое и элегантное приложение-счетчик для отработки основ.

---


### 🐍 Змейка, поедающая мои коммиты

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/SannikovVladislav/SannikovVladislav/output/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/SannikovVladislav/SannikovVladislav/output/github-contribution-grid-snake.svg">
  <img alt="Contribution Animation" src="https://raw.githubusercontent.com/SannikovVladislav/SannikovVladislav/output/github-contribution-grid-snake.svg">
</picture>
